...
for f, a, n  in zip(functions,args,names):
    ...
    np.savetxt("results/{}iterations_l1-metric_{}_test_powers.txt".format(mc_iterations, n),np.array(test_powers))
    ...
    plt.savefig("results/{}-powers{}.pdf".format(n,mc_iterations))
    plt.show()
    plt.clf()
    plt.close()
